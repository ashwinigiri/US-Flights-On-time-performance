class Usmap{
    //Function constructor starts here
    constructor(stats){
        this.stats = stats;
    ...
    }//function constructor ends here

    //function import data starts here
    importData(airports) {
        ...
        for (let i = 0; i < this.airports.length; i++) {
            if (this.projection([this.airports[i].lon, this.airports[i].lat]) == null) {
                this.airports.splice(i, 1); //splice function slices the array and saves the changes in the same array
                i--;
            }
            else {
    ...
    }//function importdata ends here

    //function makemap starts here
    makeMap(){
        let projection = this.projection;
        //Set map to be responsive
        let usmap = d3.select("#usmap")
            .attr("preserveAspectRatio", "xMinYMin meet") //some svg property https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/preserveAspectRatio
